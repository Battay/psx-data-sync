...
def test_defaults_resolve_correctly() -> None:
    ...
    assert settings.repair_staging_dir == Path("data/state/repair_staging")
    assert settings.max_rechecks_per_date_per_run == 1
    assert settings.reconciliation_cooldown_seconds == 86_400
...
def test_environment_overrides_are_typed(tmp_path: Path) -> None:
    settings = Settings.from_env(
        {
            ...
            "PSX_REPAIR_STAGING_DIR": str(tmp_path / "staging"),
            "PSX_RANGE_WORKERS": "2",
            "PSX_MAX_RECHECKS_PER_DATE_PER_RUN": "2",
            "PSX_RECONCILIATION_COOLDOWN_SECONDS": "3600",
        }
    )
    ...
    assert settings.repair_staging_dir == tmp_path / "staging"
    assert settings.range_workers == 2
    assert settings.max_rechecks_per_date_per_run == 2
    assert settings.reconciliation_cooldown_seconds == 3600
...
def test_invalid_environment_override_is_rejected() -> None:
    ...
    with pytest.raises(
        ValueError,
        match=f"cannot exceed {MAX_RECHECKS_PER_DATE_PER_RUN}",
    ):
        Settings.from_env({"PSX_MAX_RECHECKS_PER_DATE_PER_RUN": "6"})

    with pytest.raises(ValueError, match="cannot be negative"):
        Settings.from_env({"PSX_RECONCILIATION_COOLDOWN_SECONDS": "-1"})
